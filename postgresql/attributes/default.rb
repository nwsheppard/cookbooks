...
when "redhat", "centos", "scientific", "oracle"
  ...
  if node['platform_version'].to_f >= 6.0
    ...
    default['postgresql']['server']['packages'] = %w{postgresql-server}
    default['postgresql']['contrib']['packages'] = %w{postgresql-contrib}
  elsif node['postgresql']['version'].to_f == 8.1
...
when 'rhel', 'fedora', 'suse'
  if node['postgresql']['version'].to_f >= 8.3
    ...
    default['postgresql']['config']['listen_addresses'] = 'localhost'
    default['postgresql']['config']['max_connections'] = 100
    default['postgresql']['config']['shared_buffers'] = '1024'
    default['postgresql']['config']['log_directory'] = 'pg_log'
    default['postgresql']['config']['log_filename'] = 'postgresql-%a.log'
    default['postgresql']['config']['log_truncate_on_rotation'] = true
    default['postgresql']['config']['log_rotation_age'] = '1440'
    default['postgresql']['config']['log_rotation_size'] = 0
    default['postgresql']['config']['datestyle'] = 'iso, mdy'
    default['postgresql']['config']['lc_messages'] = 'en_US.UTF-8'
    default['postgresql']['config']['lc_monetary'] = 'en_US.UTF-8'
...
when 'rhel', 'fedora', 'suse'
    if node['postgresql']['version'].to_f >= 8.3
        ...
        {:type => 'local', :db => 'all', :user => 'all', :addr => nil, :method => 'ident sameuser'},
        {:type => 'host', :db => 'all', :user => 'all', :addr => '127.0.0.1/32', :method => 'trust'},
        {:type => 'host', :db => 'all', :user => 'all', :addr => '::1/128', :method => 'ident sameuser'}
      ]
    end
end
...
default['postgresql']['pgdg']['repo_rpm_url'] = {
  ...
  "8.1" => {
    "centos" => {
      "5" => {
        "i386" => "http://yum.postgresql.org/8.1/redhat/rhel-5-i386/pgdg-centos-8.1-5.noarch.rpm",
        "x86_64" => "http://yum.postgresql.org/8.1/redhat/rhel-5-x86_64/pgdg-centos-8.1-5.noarch.rpm"
      },
      "4" => {
        "i386" => "http://yum.postgresql.org/8.1/redhat/rhel-4-i386/pgdg-centos-8.1-5.noarch.rpm",
        "x86_64" => "http://yum.postgresql.org/8.1/redhat/rhel-4-x86_64/pgdg-centos-8.1-5.noarch.rpm"
      }
    },
    "redhat" => {
      "5" => {
        "i386" => "http://yum.postgresql.org/8.1/redhat/rhel-5-i386/pgdg-redhat-8.1-5.noarch.rpm",
        "x86_64" => "http://yum.postgresql.org/8.1/redhat/rhel-5-x86_64/pgdg-redhat-8.1-5.noarch.rpm"
      },
      "4" => {
        "i386" => "http://yum.postgresql.org/8.1/redhat/rhel-4-i386/pgdg-redhat-8.1-5.noarch.rpm",
        "x86_64" => "http://yum.postgresql.org/8.1/redhat/rhel-4-x86_64/pgdg-redhat-8.1-5.noarch.rpm"
      }
    }
  },
};
